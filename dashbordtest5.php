...
if ($response === FALSE) {
    die('Error occurred while fetching user profile data');
}
...
if ($response === FALSE) {
    die('Error occurred while fetching activity data');
}

$activityData = json_decode($response, true);

// Controleer of de data correct is opgehaald
if (isset($activityData['activities-log'])) {
...
<body>
    ...
    <div class="header">
        ...
        <p>Welcome, <?php echo isset($userData['user']['fullName']) ? $userData['user']['fullName'] : 'User'; ?></p>
    </div>
